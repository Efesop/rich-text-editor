...
import { attachmentIdsInPage } from './attachmentRefs.js'
...
// The relay answers for at most this many ids per request.
export const MAX_EXISTS_IDS = 200

/**
 * Ask the relay which of these attachments it already holds, so only the
 * missing ones are uploaded. The check doesn't spend the attachment transfer
 * allowance.
 *
 * @param {Object} args
 * @param {string[]} args.ids - up to MAX_EXISTS_IDS attachment ids
 * @param {Object} args.credentials
 * @param {(url:string, init:RequestInit) => Promise<Response>} [args.fetch]
 * @returns {Promise<{ok: boolean, present?: string[], missing?: string[], errorCode?: string, status?: number, retryAfterMs?: number|null}>}
 *   errorCode 'not-found' means the relay predates this check.
 */
export async function attachmentsExist ({
  ids,
  credentials,
  fetch: fetchFn = (typeof globalThis !== 'undefined' ? globalThis.fetch : null)
}) {
  if (!Array.isArray(ids) || ids.length === 0 || ids.length > MAX_EXISTS_IDS || !ids.every(id => typeof id === 'string' && id)) {
    throw new Error(`attachmentsExist: ids must be 1 to ${MAX_EXISTS_IDS} attachment ids`)
  }
  if (!credentials || !credentials.vaultKeyBytes) {
    throw new Error('attachmentsExist: credentials required')
  }
  if (typeof fetchFn !== 'function') {
    throw new Error('attachmentsExist: fetch unavailable')
  }

  const { vaultKeyBytes, vaultId, deviceId, relayUrl } = credentials
  const httpUrl = relayUrl.replace(/^wss?:\/\//, m => m === 'wss://' ? 'https://' : 'http://')
  const path = '/sync/attachments/exists'
  const headers = await buildSyncHeaders(vaultKeyBytes, {
    vaultId, deviceId, timestamp: Date.now(), method: 'POST', path,
    contentType: 'application/json'
  })
  ...
  let response
  try {
    response = await fetchFn(httpUrl + path, { method: 'POST', headers, body: JSON.stringify({ ids }) })
  } catch (err) {
    return { ok: false, errorCode: 'network', message: err.message }
  }
  ...
  if (!body || !Array.isArray(body.present) || !Array.isArray(body.missing)) {
    return { ok: false, errorCode: 'invalid-response' }
  }
  return { ok: true, present: body.present, missing: body.missing }
}
...
/**
 * Attachment IDs a page's content refers to: file blocks, and photos stored
 * as attachments (lib/attachmentRefs.js).
 *
...
export function extractAttachmentIds (page) {
  return attachmentIdsInPage(page)
}
