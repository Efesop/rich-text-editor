import mobileStorage from './mobileStorage.js'
import { LEGACY_VERSIONS_PREFIX, migrateLegacyVersions } from './legacyStorageMigration.js'

// IndexedDB when running as an installed PWA or inside the Capacitor app —
// the same check lib/storage.js uses. Capacitor's WebView matches neither
// `navigator.standalone` nor the standalone display mode, so without the
// Capacitor check the iOS app fell through to localStorage, which iOS purges.
const isPWA = typeof window !== 'undefined' &&
  (window.navigator.standalone ||
   window.matchMedia('(display-mode: standalone)').matches ||
   !!window.Capacitor?.isNativePlatform?.())

const MAX_VERSIONS = 10

// Version history an earlier iOS build left in localStorage moves to
// IndexedDB once per launch, before the first IndexedDB read.
let legacyMigration = null
function migrateLegacyOnce () {
  if (!isPWA || typeof localStorage === 'undefined') return Promise.resolve()
  if (!legacyMigration) {
    legacyMigration = migrateLegacyVersions({ storage: localStorage, idb: mobileStorage, maxVersions: MAX_VERSIONS })
      .catch(err => { console.error('Moving version history out of localStorage failed', err) })
  }
  return legacyMigration
}

function readLegacyVersions (pageId) {
  try {
    const raw = localStorage.getItem(LEGACY_VERSIONS_PREFIX + pageId)
    const parsed = raw ? JSON.parse(raw) : []
    return Array.isArray(parsed) ? parsed : []
  } catch {
    return []
  }
}

// Content hash for version dedup — uses SHA-256 via Web Crypto for collision resistance
async function contentHash (str) {
  try {
    const buffer = new TextEncoder().encode(str)
    const hashBuffer = await crypto.subtle.digest('SHA-256', buffer)
    const hashArray = new Uint8Array(hashBuffer)
    return Array.from(hashArray.slice(0, 8), b => b.toString(16).padStart(2, '0')).join('')
  } catch {
    // Fallback for environments without crypto.subtle (e.g. non-HTTPS)
    let hash = 5381
    for (let i = 0; i < str.length; i++) {
      hash = ((hash << 5) + hash + str.charCodeAt(i)) | 0
    }
    return hash.toString(36)
  }
}

export async function readVersions (pageId) {
  // Electron (desktop)
  if (typeof window !== 'undefined' && window.electron?.invoke) {
    return await window.electron.invoke('read-versions', pageId)
  }

  // PWA (mobile) — use IndexedDB
  if (isPWA) {
    await migrateLegacyOnce()
    const stored = await mobileStorage.readVersions(pageId)
    // Still in localStorage if its move hasn't completed.
    return stored.length > 0 ? stored : readLegacyVersions(pageId)
  }

  // Web browser fallback — localStorage
  try {
    const raw = localStorage.getItem(`dash-versions-${pageId}`)
    return raw ? JSON.parse(raw) : []
  } catch (err) {
    console.error('readVersions fallback failed', err)
    return []
  }
}

export async function saveVersions (pageId, versions) {
  // Electron (desktop)
  if (typeof window !== 'undefined' && window.electron?.invoke) {
    return await window.electron.invoke('save-versions', pageId, versions)
  }

  // PWA (mobile)
  if (isPWA) {
    return await mobileStorage.saveVersions(pageId, versions)
  }

  // Web browser fallback
  try {
    localStorage.setItem(`dash-versions-${pageId}`, JSON.stringify(versions))
    return { success: true }
  } catch (err) {
    console.error('saveVersions fallback failed', err)
    return { success: false }
  }
}

export async function deleteVersions (pageId) {
  // Electron (desktop)
  if (typeof window !== 'undefined' && window.electron?.invoke) {
    return await window.electron.invoke('delete-versions', pageId)
  }

  // PWA (mobile)
  if (isPWA) {
    // Remove any localStorage copy too, so the migration can't bring it back.
    try { localStorage.removeItem(LEGACY_VERSIONS_PREFIX + pageId) } catch { /* unavailable */ }
    return await mobileStorage.deleteVersions(pageId)
  }

  // Web browser fallback
  try {
    localStorage.removeItem(`dash-versions-${pageId}`)
    return { success: true }
  } catch (err) {
    console.error('deleteVersions fallback failed', err)
    return { success: false }
  }
}

// Throttle: minimum 30 seconds between version captures per page
const MIN_CAPTURE_INTERVAL = 30 * 1000
const lastCaptureTime = new Map()

// Capture a version snapshot if content has changed and enough time has passed
// Returns true if a version was saved, false if skipped
export async function captureVersion (pageId, blocks) {
  try {
    // Throttle — skip if captured too recently
    const now = Date.now()
    const lastTime = lastCaptureTime.get(pageId) || 0
    if (now - lastTime < MIN_CAPTURE_INTERVAL) {
      return false
    }

    const contentStr = JSON.stringify(blocks)
    const hash = await contentHash(contentStr)

    const existing = await readVersions(pageId)

    // Skip if latest version has same hash
    if (existing.length > 0 && existing[0].contentHash === hash) {
      return false
    }

    const snapshot = {
      timestamp: new Date().toISOString(),
      contentHash: hash,
      blocks
    }

    const updated = [snapshot, ...existing].slice(0, MAX_VERSIONS)
    await saveVersions(pageId, updated)
    lastCaptureTime.set(pageId, now)
    return true
  } catch (err) {
    console.error('captureVersion failed', err)
    return false
  }
}
