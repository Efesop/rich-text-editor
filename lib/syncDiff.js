 ...
 *   - emoji (folders)
 *   - templates (folders: marks the templates folder)
...
function pagesShallowEqual (a, b) {
  ...
  if (a.type === 'folder') {
    ...
    if (Boolean(a.templates) !== Boolean(b.templates)) return false
  ...
  // `restoredAt` is the explicit-restore signal that syncPull uses to
  // decide whether to resurrect a peer-trashed page. Treat changes as
  // sync-relevant so a restore actually emits an upsert envelope.
  if ((a.restoredAt || null) !== (b.restoredAt || null)) return false
  // Emoji on notes — was previously only checked on the folder path,
  // so emoji edits to notes never propagated to peers (latent bug
  // surfaced in build-22 audit).
  if ((a.emoji || null) !== (b.emoji || null)) return false
  // Pinning and unpinning reach other devices too
  if ((a.pinnedAt || null) !== (b.pinnedAt || null)) return false
...
function shallowJsonEqual (a, b) {
  if (a === b) return true
  if (!a || !b) return a === b
  // Length-first early-out: stringify both sides ONCE and compare lengths
  // before comparing the full string. Catches the >95% common case of
  // unchanged content at near-zero cost while still being correctness-
  // equivalent to the old "stringify both, compare strings" path. On a
  // 200-page vault with 50 KB pages, the previous full-stringify-twice
  // path ran the encoder for ~10 MB of JSON per save — visible CPU
  // burn during typing on iOS. Two pages happening to have identical
  // byte length but different content fall through to the full string
  // compare, so correctness is preserved.
...
export function buildManifestPayload (pages, tags) {
  ...
  const folders = pages
    ...
    .map(f => ({
      ...
      ...(f.templates === true ? { templates: true } : {})
    }))
