 ...
 */
// The relay rejects a second request whose auth tag equals the last one it
// saw for the device ("Replayed auth token"). The tag is an HMAC over
// vaultId/deviceId/timestamp/method/path, so two requests to the same path
// in the same millisecond collide — the settings panel's usage refresh did
// exactly that and burned a 401 + re-register every cycle. Keep timestamps
// strictly increasing within this process.
let lastSyncTimestamp = 0
function uniqueTimestamp (requested) {
  const t = Math.max(Number.isInteger(requested) && requested > 0 ? requested : Date.now(), lastSyncTimestamp + 1)
  lastSyncTimestamp = t
  return t
}

export async function buildSyncHeaders (vaultKeyBytes, { vaultId, deviceId, timestamp, method, path, contentType }) {
  timestamp = uniqueTimestamp(timestamp)
