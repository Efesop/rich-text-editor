  ...
  async savePages(pages) {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        // Use put() to upsert each page — avoids clear() which risks data loss on crash
        let completed = 0
        const total = pages.length
        const pageIds = new Set(pages.map(p => p.id))

        if (total === 0) {
          // No pages — clear the store
          const clearRequest = store.clear()
          clearRequest.onsuccess = () => resolve({ success: true })
        ...
        for (const page of pages) {
          ...
          putRequest.onerror = () => {
            const isQuotaError = putRequest.error?.name === 'QuotaExceededError'
            console.error('Error saving page to IndexedDB:', putRequest.error)
            if (isQuotaError) {
              console.warn('IndexedDB quota exceeded — falling back to localStorage')
            }
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        for (const tag of tags) {
          ...
          putRequest.onerror = () => {
            console.error('Error saving tag to IndexedDB:', putRequest.error)
            this.saveToLocalStorage('tags', tags)
            resolve({ success: true })
          }
        }
  ...
  async saveAttachment (attachmentId, arrayBuffer) {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        const request = store.put(arrayBuffer, attachmentId)
        request.onsuccess = () => resolve({ success: true })
  ...
  async saveVersions (pageId, snapshots) {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        const store = transaction.objectStore('versions')
        const request = store.put({ pageId, snapshots })
        request.onsuccess = () => resolve({ success: true })
        request.onerror = () => reject(request.error)
      })
    } catch (error) {
  ...
  async saveVaultMetadata (metadata) {
    try {
      ...
      return new Promise((resolve) => {
        ...
        const store = transaction.objectStore('vaultMetadata')
        const request = store.put({ key: 'singleton', metadata })
        request.onsuccess = () => resolve({ success: true })
  ...
  async saveSyncQueue (entries) {
    try {
      ...
      return new Promise((resolve) => {
        ...
        const store = transaction.objectStore('syncQueue')
        const request = store.put({ key: 'singleton', entries })
        request.onsuccess = () => resolve({ success: true })
        request.onerror = () => resolve({ success: false })
      })
    } catch {
