...
class MobileStorage {
  constructor() {
    ...
    // Includes the Capacitor app, which matches neither standalone check.
    this.isPWA = typeof window !== 'undefined' && (window.navigator.standalone || window.matchMedia('(display-mode: standalone)').matches || !!window.Capacitor?.isNativePlatform?.())
  }
  ...
  // Existence only, without reading the bytes.
  async hasAttachment (attachmentId) {
    try {
      await this.init()
      return new Promise((resolve) => {
        const transaction = this.db.transaction(['attachments'], 'readonly')
        const request = transaction.objectStore('attachments').count(attachmentId)
  ...
  // --- Attachment transfers (lib/attachmentTransferQueue.js) ---
  // Kept in the syncQueue store beside the push queue, under their own key.
  async readAttachmentTransfers () {
    try {
      await this.init()
      return new Promise((resolve) => {
        const transaction = this.db.transaction(['syncQueue'], 'readonly')
        const request = transaction.objectStore('syncQueue').get('attachment-transfers')
  ...
  async saveAttachmentTransfers (state) {
    try {
      await this.init()
      return new Promise((resolve) => {
        const transaction = this.db.transaction(['syncQueue'], 'readwrite')
        transaction.objectStore('syncQueue').put({ key: 'attachment-transfers', state })
        transaction.oncomplete = () => resolve({ success: true })
        transaction.onerror = () => resolve({ success: false, error: transaction.error?.message })
        transaction.onabort = () => resolve({ success: false, error: transaction.error?.message })
      })
