...
// Capacitor SecureStorage — top-level static import so webpack bundles
// it into the main chunk. Resolves to a plugin object on Capacitor
// builds, undefined on Electron / browser (handled at call sites).
import { SecureStoragePlugin as SecureStoragePluginImport } from 'capacitor-secure-storage-plugin'
 ...
 * @property {'app-lock'|'passphrase'|'safe-storage'|'ios-keychain'} keyWrapMethod
 */
...
export function validateMetadata (m) {
  ...
  if (m.syncEnabled === true) {
    ...
    if (!['app-lock', 'passphrase', 'safe-storage', 'ios-keychain'].includes(m.keyWrapMethod)) {
...
export function createVaultStore (backend) {
  ...
  return {
     ...
     */
    async createVault ({ deviceName, relayUrl, wrapMethod, appLockKey, passphrase, safeStorageStore, iosKeychainStore }) {
      ...
      if (wrapMethod === 'app-lock') {
        ...
        method = 'safe-storage'
      } else if (wrapMethod === 'ios-keychain') {
        if (typeof iosKeychainStore !== 'function') {
          throw new Error("createVault: wrapMethod='ios-keychain' requires iosKeychainStore callback")
        }
        await iosKeychainStore(rawKey)
        wrapped = { method: 'ios-keychain', ref: 'vault-key' }
        method = 'ios-keychain'
      } else {
    ...
    /**
     * Adopt an EXISTING vault from a QR-pair packet. Used on the new
     * (guest) device after a successful AcceptPairModal flow. The packet
     * contains the existing vault's vaultId + raw vault key + relay URL
     * + list of paired devices. We mint a fresh deviceId for this device,
     * wrap the vault key under the chosen method, and produce metadata
     * ready to save.
     *
     * @param {Object} opts
     * @param {Object} opts.packet - { vaultId, vaultKey: number[], relayUrl, pairedDevices }
     * @param {string} opts.deviceName - user-friendly device name
     * @param {'app-lock'|'passphrase'|'safe-storage'} opts.wrapMethod
     * @param {Object} [opts.appLockKey]
     * @param {string} [opts.passphrase]
     * @param {(rawKey: Uint8Array) => Promise<void>} [opts.safeStorageStore]
     * @returns {Promise<{metadata: VaultMetadata, vaultKey: Uint8Array}>}
     */
    async adoptVault ({ packet, deviceName, wrapMethod, appLockKey, passphrase, safeStorageStore, iosKeychainStore }) {
      if (!packet || typeof packet !== 'object') {
        throw new Error('adoptVault: packet required')
      }
      if (typeof packet.vaultId !== 'string' || !packet.vaultId) {
        throw new Error('adoptVault: packet.vaultId required')
      }
      if (!Array.isArray(packet.vaultKey) || packet.vaultKey.length !== 32) {
        throw new Error('adoptVault: packet.vaultKey must be 32-byte array')
      }
      if (typeof packet.relayUrl !== 'string' || !/^wss?:\/\//.test(packet.relayUrl)) {
        throw new Error('adoptVault: packet.relayUrl invalid')
      }
      if (typeof deviceName !== 'string' || !deviceName) {
        throw new Error('adoptVault: deviceName required')
      }

      const rawKey = new Uint8Array(packet.vaultKey)
      let wrapped, method
      if (wrapMethod === 'app-lock') {
        if (!appLockKey?.key || !appLockKey?.salt) throw new Error("adoptVault: app-lock requires appLockKey")
        wrapped = await wrapVaultKey(rawKey, appLockKey.key, appLockKey.salt)
        method = 'app-lock'
      } else if (wrapMethod === 'passphrase') {
        if (typeof passphrase !== 'string' || !passphrase) throw new Error("adoptVault: passphrase required")
        wrapped = await wrapVaultKeyWithPassphrase(rawKey, passphrase)
        method = 'passphrase'
      } else if (wrapMethod === 'safe-storage') {
        if (typeof safeStorageStore !== 'function') throw new Error("adoptVault: safeStorageStore callback required")
        await safeStorageStore(rawKey)
        wrapped = { method: 'safe-storage', ref: 'vault-key' }
        method = 'safe-storage'
      } else if (wrapMethod === 'ios-keychain') {
        if (typeof iosKeychainStore !== 'function') throw new Error("adoptVault: iosKeychainStore callback required")
        await iosKeychainStore(rawKey)
        wrapped = { method: 'ios-keychain', ref: 'vault-key' }
        method = 'ios-keychain'
      } else {
        throw new Error(`adoptVault: unknown wrapMethod ${wrapMethod}`)
      }

      const now = new Date().toISOString()
      const metadata = {
        ...emptyMetadata(),
        vaultId: packet.vaultId,
        ...
        createdAt: now,
        lastPairedAt: now,
        pairedDevices: Array.isArray(packet.pairedDevices) ? packet.pairedDevices : [],
        lastSyncedVersion: {},
        cursorVersion: 0,
        wrappedVaultKey: wrapped,
        keyWrapMethod: method
     ...
     */
    async unlock (metadata, { appLockKey, passphrase, safeStorageRetrieve, iosKeychainRetrieve } = {}) {
      validateMetadata(metadata)
      ...
      if (metadata.keyWrapMethod === 'app-lock') {
        ...
        raw = retrieved
      } else if (metadata.keyWrapMethod === 'ios-keychain') {
        if (typeof iosKeychainRetrieve !== 'function') {
          throw new Error("unlock: keyWrapMethod='ios-keychain' requires iosKeychainRetrieve callback")
        }
        const retrieved = await iosKeychainRetrieve()
        if (!(retrieved instanceof Uint8Array) || retrieved.length !== 32) {
          throw new DecryptionError('unlock: iosKeychain returned invalid vault key')
        }
        raw = retrieved
      } else {
...
export function createPwaVaultBackend (mobileStorageInstance) {
  ...
  return {
    ...
    async clear () {
      await mobileStorageInstance.clearVaultMetadata()
      // Also wipe the iOS Keychain copy of the vault key (no-op off-native).
      // Mirrors the Electron backend's vault-key-delete: disabling sync must
      // not leave the raw vault key on the device.
      try { await iosKeychainDeleteVaultKey() } catch { /* ignore */ }
    }
  }
}
...
/**
 * iOS Keychain helpers (Capacitor) — store / retrieve the raw vault key
 * in iOS Keychain via `capacitor-secure-storage-plugin`. Items are
 * accessible after first device unlock, encrypted at rest by iOS.
 *
 * Plugin is imported statically (top of file) so webpack bundles it
 * into the main chunk. Dynamic import() chunks were 404ing on Capacitor
 * iOS — the webview's `http://localhost` scheme couldn't resolve
 * relative chunk URLs, leaving adoptVault silently hanging on `ss.set`.
 *
 * Biometric gating is intentionally deferred — `capacitor-secure-storage-plugin`
 * stores items with `kSecAttrAccessibleAfterFirstUnlock`, which is fine
 * for a sync vault key that follows app-lock semantics. A separate
 * Face ID prompt was attempted via `@aparajita/capacitor-biometric-auth`
 * but introduced pair-flow regressions; revisit when sync is stable.
 */
const KEYCHAIN_KEY = 'dash-vault-key'

// Resolved at module load via the static ESM import at the top of this
// file. Tracked as nullable because the import may not resolve to a
// working plugin object on non-Capacitor builds.
let SecureStoragePlugin = SecureStoragePluginImport
...
export async function iosKeychainStoreVaultKey (rawKey) {
  const ss = getSecureStorage()
  if (!ss) throw new Error('iosKeychainStoreVaultKey: Capacitor SecureStorage not available')
  if (!(rawKey instanceof Uint8Array) || rawKey.length !== 32) {
    throw new Error('iosKeychainStoreVaultKey: rawKey must be 32-byte Uint8Array')
  }
  let bin = ''
  for (let i = 0; i < rawKey.length; i++) bin += String.fromCharCode(rawKey[i])
  const b64 = typeof btoa === 'function' ? btoa(bin) : Buffer.from(rawKey).toString('base64')
  await ss.set({ key: KEYCHAIN_KEY, value: b64 })
}

export async function iosKeychainRetrieveVaultKey () {
  const ss = getSecureStorage()
  if (!ss) throw new Error('iosKeychainRetrieveVaultKey: Capacitor SecureStorage not available')
  let b64
  try {
    const result = await ss.get({ key: KEYCHAIN_KEY })
    b64 = result?.value
  } catch (err) {
...
export async function iosKeychainDeleteVaultKey () {
  const ss = getSecureStorage()
  if (!ss) return
  try { await ss.remove({ key: KEYCHAIN_KEY }) } catch { /* not found is fine */ }
}
