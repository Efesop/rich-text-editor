...
export function createSyncQueue (opts) {
  ...
  async function persist () {
    ...
    try {
      // Strip transient fields; keep enough to replay on restart.
      ...
      await persistBackend.write(persistable)
    } catch (err) {
  ...
  function takeBatch () {
    ...
    for (const e of entries) {
      if (batch.length >= BATCH_COUNT) break
  ...
  async function flush () {
    ...
    if (result.errorCode === 'too-large' && batch.length > 1) {
      // Try smaller batch — halve and retry
      // (we don't actually modify BATCH_COUNT; we just leave the head and
      // retry — the next flush will take a smaller batch since we increment
      // attempts and the entries are still at head.)
      // Easier: bump attempts on every entry then schedule retry with backoff
      for (const e of batch) e.attempts++
