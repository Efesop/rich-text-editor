...
class LocalSyncManager {
  ...
  async init() {
    ...
    await this.initSyncKeys()
    this.initialized = true
  }

  generateDeviceId() {
    // Return fallback ID if not in browser
    if (typeof window === 'undefined' || typeof localStorage === 'undefined') {
      return 'dash-server-fallback'
    }
  ...
  getDeviceName() {
    // Return fallback name if not in browser
  ...
  async initSyncKeys() {
    // Skip key initialization if not in browser
    if (typeof window === 'undefined' || typeof localStorage === 'undefined') {
      this.syncKey = null
      return
    }
    ...
    try {
      // Generate new sync key for this device (don't store CryptoKey in localStorage)
      this.syncKey = await generateKey()
    } catch (e) {
      console.error('Failed to generate sync key:', e)
      this.syncKey = null
    }
  }
