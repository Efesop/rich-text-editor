...
import { resolveRelayUrl, toHttpUrl } from './relayHosts.js'

// Build-time relay URL, resolved per call to whichever public alias answers
// from this device (see relayHosts.js — some phones block *.deno.net).
const RELAY_ENV = process.env.NEXT_PUBLIC_RELAY_URL || 'wss://dash-relay.efesop.deno.net'
async function relayHttp () {
  return toHttpUrl(await resolveRelayUrl(RELAY_ENV))
}
...
export async function requestCode (email) {
  ...
  const res = await fetch(`${await relayHttp()}/auth/code/request`, {
    method: 'POST',
...
export async function verifyCode (email, code) {
  ...
  const res = await fetch(`${await relayHttp()}/auth/code/verify`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function signOut () {
  ...
  if (token) {
    try {
      await fetch(`${await relayHttp()}/auth/signout`, {
        method: 'POST',
...
export async function getMe () {
  ...
  try {
    const res = await fetch(`${await relayHttp()}/auth/me`, {
      headers: { Authorization: `Bearer ${token}` }
    })
    if (res.status === 401) {
      clearSession()
