...
// Compact base64 helpers (works in browsers, Electron, Capacitor WebView).
// Used to keep the QR pair packet small enough to fit in a low-density
// QR. Decimal-array encoding (`Array.from(uint8)`) inflates 3-4x —
// scanner-unscannable at 280 px modal size.
...
export async function encryptEnvelope (envelope, vaultKeyCrypto) {
  ...
  // Gzip the payload before encryption. Block-structured note content
  // (Editor.js JSON: lots of repeated keys, whitespace, span markers)
  // compresses 5-10×, which keeps large notes under Deno KV's 64 KB
  // per-value cap. Compression happens BEFORE encryption so ciphertext
  // size mirrors compressed plaintext (encrypting compressible data
  // post-encryption gains nothing — ciphertext is high-entropy).
  const plaintext = await gzipBytes(json)
  const ciphertext = new Uint8Array(
    await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, vaultKeyCrypto, plaintext)
  )
  return {
    v: 2,                      // bumped: v2 indicates gzip(plaintext) before encrypt
    cipher: 'AES-GCM-256',
    iv: Array.from(iv),
...
export async function decryptEnvelope (payload, vaultKeyCrypto) {
  ...
  try {
    ...
    // v2 envelopes are gzip-compressed before encryption (see encryptEnvelope).
    // v1 envelopes are raw JSON — decrypt path branches on schemaVersion.
    const json = payload.v === 2 ? await gunzipBytes(decrypted) : decrypted
 ...
 */
export async function encryptPairPacket (packet, pairCode, options = {}) {
  ...
  const schemaVersion = options.schemaVersion === 1 ? 1 : 2
  ...
  // Schema v2 (default): base64 strings — cuts payload ~3.5x so the
  // QR fits in ~70×70 modules and scans cleanly. Schema v1 (opt-in
  // via options.schemaVersion): decimal-number arrays. Used as a
  // bridge for OLD iOS installs whose stale-cache decoder doesn't
  // know v=2 yet — Mac generates v=1 alongside v=2, surfaces v=1 on
  // the paste-link path so old devices can pair until they refresh.
  if (schemaVersion === 1) {
    return {
      v: 1,
      kdf: 'PBKDF2-SHA256',
...
export async function decryptPairPacket (payload, pairCode) {
  ...
  // v=1: salt/iv/data are decimal-number arrays. v=2: base64 strings.
  const isV2 = payload.v === 2
  const fieldsValid = isV2
    ? (typeof payload.salt === 'string' && typeof payload.iv === 'string' && typeof payload.data === 'string')
    : (Array.isArray(payload.salt) && Array.isArray(payload.iv) && Array.isArray(payload.data))
  if (!fieldsValid) {
  ...
  try {
    const salt = isV2 ? b64ToBytes(payload.salt) : new Uint8Array(payload.salt)
    const iv = isV2 ? b64ToBytes(payload.iv) : new Uint8Array(payload.iv)
    const data = isV2 ? b64ToBytes(payload.data) : new Uint8Array(payload.data)
    const baseKey = await crypto.subtle.importKey(
      'raw',
      textEncoder.encode(pairCode),
      { name: 'PBKDF2' },
...
// =============================================================================
// Binary blob encryption — for attachments (Phase 2.6)
// =============================================================================

/**
 * Encrypt a Uint8Array under the vault key. Wire format is just
 * [12B IV][ciphertext including AES-GCM tag] — no JSON wrapper, since
 * attachments are pre-binary and we want to minimize overhead. The server
 * receives this as a base64 blob and stores opaque bytes.
 *
 * @param {Uint8Array} bytes - plaintext
 * @param {CryptoKey} vaultCryptoKey - imported vault key
 * @returns {Promise<Uint8Array>} concatenated [iv | ciphertext]
 */
export async function encryptBytes (bytes, vaultCryptoKey) {
  if (!(bytes instanceof Uint8Array)) {
    throw new Error('encryptBytes: bytes must be Uint8Array')
  }
  if (!(vaultCryptoKey instanceof CryptoKey)) {
    throw new Error('encryptBytes: vaultCryptoKey must be a CryptoKey')
  }
  const iv = crypto.getRandomValues(new Uint8Array(12))
  const ciphertext = new Uint8Array(
    await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, vaultCryptoKey, bytes)
  )
...
/**
 * Decrypt an attachment blob produced by encryptBytes. Throws DecryptionError
 * on wrong key, corruption, or input shorter than 12 bytes.
 *
 * @param {Uint8Array} blob - [iv | ciphertext]
 * @param {CryptoKey} vaultCryptoKey
 * @returns {Promise<Uint8Array>} decrypted bytes
 */
export async function decryptBytes (blob, vaultCryptoKey) {
  if (!(blob instanceof Uint8Array) || blob.length < 13) {
    throw new DecryptionError('decryptBytes: blob must be Uint8Array of length >= 13')
  }
  if (!(vaultCryptoKey instanceof CryptoKey)) {
    throw new Error('decryptBytes: vaultCryptoKey must be a CryptoKey')
  }
  ...
  try {
    ...
    if (error.name === 'OperationError') {
      throw new DecryptionError('decryptBytes: wrong vault key (AES-GCM auth failed)')
    }
    throw new DecryptionError(`decryptBytes: ${error.message}`, error)
  }
}
