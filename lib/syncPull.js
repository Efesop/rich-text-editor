  ...
  for (const env of envelopes) {
    ...
    if (env.envelopeType === 'folder' || env.resourceType === 'folder') {
      ...
      if (existing && existing.type === 'folder' && Array.isArray(existing.pages)) {
        ...
        // A device on an older version drops the templates flag when it saves
        // the folder. Keep the flag, so the Templates folder stays one.
        const keepTemplates = existing.templates === true && incoming.templates === undefined
        pagesById.set(incoming.id, { ...incoming, pages: [...incomingPages, ...localOnly], ...(keepTemplates ? { templates: true } : {}) })
      } else {
        pagesById.set(incoming.id, incoming)
      }
      applied.push(incoming.id)
...
function classifyHttpStatus (status) {
  ...
  if (status === 402) return 'subscription-required' // relay entitlement gate (no plan on this email)
