 ...
 * @returns {Promise<{outcome: 'moved'|'unchanged'|'restored'|'failed', reason: string|null, photos: number, bytes: number, stayedInline: number}>}
 */
export async function migrateNotePhotos ({
  ...
  const report = (outcome, reason, extra = {}) => ({ outcome, reason, photos: 0, bytes: 0, stayedInline: 0, ...extra })
  ...
  for (const photo of photos) {
    ...
    if (!decoded || !sniffImageType(decoded.bytes)) continue
    try {
  ...
  if (moved.length === 0) return report('unchanged', tooLarge ? 'photo-too-large' : 'unreadable-photo', { stayedInline })
  if (shouldStop()) return report('unchanged', 'paused')
  ...
  await journal.clear()
  return report('moved', null, { photos: moved.length, bytes, stayedInline })
}
