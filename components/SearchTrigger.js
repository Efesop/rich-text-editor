...
export default function SearchTrigger ({ searchTerm, selectedTags = [], onClick, onClear, theme, tagColorMap = {}, placeholder = 'Search everything', large = false }) {
  ...
  return (
    <div className="relative flex items-center">
      <button
        onClick={onClick}
        className={`w-full flex items-center gap-2 pl-3 pr-3 ${large ? 'h-10 rounded-[10px] text-base' : 'py-2 rounded-lg text-sm'} border cursor-pointer transition-colors hover:ring-1 ${inputClasses} ${
      ...
      >
        <Search className={`${large ? 'h-[18px] w-[18px]' : 'h-4 w-4'} flex-shrink-0 ${iconClasses}`} />
