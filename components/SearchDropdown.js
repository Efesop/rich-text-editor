...
export default function SearchDropdown({ 
  ...
  const getInputClasses = () => {
    const baseClasses = 'w-full pl-10 pr-4 py-2 rounded-lg border text-sm focus:outline-none focus:ring-1'
  ...
  const getDropdownClasses = () => {
    const baseClasses = 'absolute top-full left-0 right-0 mt-1 rounded-lg border shadow-lg z-[60] max-h-64 overflow-auto'
  ...
  const getSuggestionClasses = (index) => {
    const baseClasses = 'w-full text-left px-3 py-2 text-sm focus:outline-none flex items-center space-x-3 transition-colors'
